...
Personal job-application automation built on top of `browser_use`. Given one or more resumes,
it finds new matching postings on Greenhouse/Lever boards, screens each one against whichever
resume fits it best, and submits an application through the real browser for anything that
passes screening.
...
1. Populate `job_agent/config.py`:
...
2. `uv run python -c "import asyncio; from job_agent.pipeline import run; asyncio.run(run())"`
...
Never hits real job boards or a real browser -- discovery/LLM/browser-use calls are all injected
via fakes at the function boundary (`adapters=`, `llm=`, `_run_apply_agent`).
